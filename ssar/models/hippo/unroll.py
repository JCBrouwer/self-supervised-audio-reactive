...
def variable_unroll_general(A, u, s, op, compose_op=None, sequential_op=None, variable=True, recurse_limit=16):
    ...
    x_1 = variable_unroll_general(
        A_10, u_10, s, op, compose_op, sequential_op, variable=variable, recurse_limit=recurse_limit
    )
...
def variable_unroll_matrix(A, u, s=None, variable=True, recurse_limit=16):
    ...
    return variable_unroll_general(
        A, u, s, op, compose_op=matmul, sequential_op=sequential_op, variable=variable, recurse_limit=recurse_limit
    )
...
def test_correctness():
    ...
    # Test utilities
    assert torch.isclose(shift_up(x), torch.Tensor([[0.0, 0.0], [1.0, 1.0], [3.0, 2.0]])).all()
    assert torch.isclose(
        interleave(x, x), torch.Tensor([[1.0, 1.0], [1.0, 1.0], [3.0, 2.0], [3.0, 2.0], [6.0, 4.0], [6.0, 4.0]])
    ).all()

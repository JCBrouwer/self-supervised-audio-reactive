...
if __name__ == "__main__":
    ...
    if not load_latents:
        ...
        for t, (t_time, t_bpm) in enumerate(raw_track_times):
            ...
            tracks.append([t, int(start_time * fac), int(stop_time * fac), t_bpm, start_time, stop_time])
            print(t, int(start_time * fac), int(stop_time * fac), t_bpm, start_time, stop_time)
    ...
    if not load_latents:
        ...
        def get_loops_per_track(loop_latents_file, tracks, num_beats, loop_smoothing, loop_factor):
            ...
            transition_window = int(round(2 * fps))
        ...
        drop_latents, drop_selection = get_loops_per_track(
            intro_file, tracks, num_beats=16, loop_smoothing=4 * smf, loop_factor=0.35
        )
        ...
        # smooth_size = 5 * smf
        # kernel = signal.hann(smooth_size)[smooth_size // 2 :]  # causal gaussian filter
        # main_weight = signal.convolve(main_weight, kernel, mode="same") / np.sum(kernel)
        for t, t_start, t_stop, t_bpm, start_time, stop_time in tracks:
            main_weight[t_start:t_stop] = gaussian_filter(
                rms[t_start:t_stop] * onset[t_start:t_stop], [2.5 * smf * 86 / t_bpm]
            )
            main_weight[t_start:t_stop] = percentile_clip(main_weight[t_start:t_stop], 99)
        main_weight = gaussian_filter(main_weight, [1 * smf * 86 / t_bpm])
        ...
        freqs = signal.resample((pitches.mean(0) + freqs), num_frames)
        freqs /= 10
        ...
        freqs = freqs % drop_selection.shape[0]
        freqs = freqs.astype(np.uint8)

        reactive_latents = drop_selection.numpy()[freqs, :, :]
        reactive_latents = gaussian_filter(reactive_latents, [5.5 * smf, 0, 0])
        ...
        ] + reactive_latents[:, main_layers[0] : main_layers[1], :] * 2.2 * main_weight
        ...
        bass_weight = gaussian_filter(bass_weight, [120 * smf])
        ...
        for t, t_start, t_stop, t_bpm, start_time, stop_time in tracks:
            drop_weight[t_start:t_stop] = percentile_clip(drop_weight[t_start:t_stop], 65)
            drop_weight[t_start:t_stop] = gaussian_filter(drop_weight[t_start:t_stop], [4 * smf])
        drop_weight = gaussian_filter(drop_weight, [1 * smf])
        ...
        latents = gaussian_filter(latents, [2.5 * smf, 0, 0])
        latents = th.from_numpy(latents).float()

        high_noise_mod = percentile_clip(main_weight.numpy() ** 2, 97)
        high_noise_mod *= 4

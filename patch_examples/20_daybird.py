...
dict(title="Drop A", start=52 * 60 + 20, end=53 * 60 + 13),
dict(title="Drop B", start=53 * 60 + 13, end=54 * 60),
...
def initialize(args):
    ...
    args.main_weight = (
        ar.compress(ar.onsets(args.audio, args.sr, args.n_frames, clip=95, smooth=2), 0.5, 0.5) * args.drop_weight
    )
...
def get_latents(selection, args):
    ...
    intro_latents = ar.slerp_loops(intro_selection, args.n_frames, args.bpm / 60 * args.duration / 64, smoothing=9)
    ...
    color_loops = ar.slerp_loops(colors, args.n_frames, args.bpm / 60 * args.duration / 8, smoothing=4)
    latents[:, 10:] = color_loops[:, 10:]

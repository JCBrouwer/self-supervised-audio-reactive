...
dict(title="Intro", start=41 * 60 + 19, end=42 * 60 + 3),
dict(title="Drop 1", start=42 * 60 + 3, end=43 * 60 + 34),
dict(title="Break", start=43 * 60 + 34, end=44 * 60 + 19),
dict(title="Drop 2", start=44 * 60 + 19, end=45 * 60 + 26),
start = 41 * 60
DROP1 = 42 * 60 + 3 - start
DROP2 = 44 * 60 + 19 - start
...
def initialize(args):
    ...
    try:
        args.drop_weight = ar.rms(args.audio, args.sr, args.n_frames, smooth=100, clip=55) ** 2
    except IndexError:
...
def get_latents(selection, args):
    ...
    reactive = ar.load_latents("/home/hans/datasets/frequent/redrocks/lats/dohr-013_reac_corrupt.npy")
    structure = ar.load_latents("/home/hans/datasets/frequent/redrocks/lats/dohr-013_struc_corrupt.npy")
    ...
    freqs = (args.focus_chroma % reactive.shape[0]).long().numpy()
    reactive_latents = torch.from_numpy(reactive.numpy()[freqs, :, :]).float()
    ...
    color_loops_drop1 = ar.slerp_loops(structure, args.n_frames, args.bpm / 60 * args.duration / 8, smoothing=8)
    color_loops_drop2 = ar.slerp_loops(reactive, args.n_frames, args.bpm / 60 * args.duration / 8, smoothing=4)
    color_loops[int(DROP1 * args.fps) : int(DROP2 * args.fps)] = color_loops_drop1[
        int(DROP1 * args.fps) : int(DROP2 * args.fps)
    ]
    color_loops[int(DROP2 * args.fps) :] = color_loops_drop2[int(DROP2 * args.fps) :]

...
if __name__ == "__main__":
    ...
    for t, (t_time, t_bpm) in enumerate(raw_tracks):
        ...
        #     stop_time - start_time,
        #     int(round(start_time * fac)) - start_frame,
        #     600,  # int(round(stop_time * fac)) - start_frame,
        # )
    ...
    if not eval(os.environ["RENDER"]):
        ...
        def get_latent_loops(base_latent_selection, loop_starting_latents, n_frames, num_loops, smoothing):
            base_latents = []
            # print(len(base_latent_selection), int(n_frames // max(1, num_loops) // len(base_latent_selection)))
        ...
        for t, t_bpm, t_dur, t_start, t_stop in tracks:
            ...
            if render_track is None:
                ...
                # print(tracks[t - 1], tracks[t + 1])
                prev_t_dur = tracks[t - 1][4] - tracks[t - 1][3] if t != 0 else 0
                next_t_dur = tracks[t + 1][4] - tracks[t + 1][3] if t != len(tracks) - 1 else 1
                t_frames = t_stop - t_start
                # print(prev_t_dur, next_t_dur)

                rms_slice = rms[t_start - prev_t_dur : t_stop + next_t_dur]
                onsets_slice = onsets[t_start - prev_t_dur : t_stop + next_t_dur]
                bass_sum_slice = bass_sum[t_start - prev_t_dur : t_stop + next_t_dur]
            else:
                if t != render_track:
                    continue
                info(rms)
            ...
            # info(wrapping_slice(intro_selection, t, 16))
            # print(prms[t]["intro_loop_smoothing"] * smf)
            intro_loops = get_latent_loops(
                base_latent_selection=wrapping_slice(intro_selection, t, 4),
            ...
            drop_loops = get_latent_loops(
                base_latent_selection=wrapping_slice(drop_selection, t, 8),
                loop_starting_latents=t,
                n_frames=t_stop - t_start,
            ...
            main_weight = percentile_clip(main_weight, prms[t]["onset_clip"])[prev_t_dur:][:t_frames]
            main_weight = th.from_numpy(main_weight)[:, None, None]
            plot_signals([main_weight])

            # %%
            # info([average_pitch_slice, pitches_slice, high_average_pitch_slice, high_pitches_slice])
            ...
            drop_weight = th.from_numpy(drop_weight)[prev_t_dur:][:t_frames, None, None]
            plot_signals([drop_weight])

            # %%
            latents = drop_weight * drop_latents + (1 - drop_weight) * intro_latents

            # if os.path.exists(f"workspace/{file_root}_color_latents.npy"):
            #     color_latents = th.from_numpy(np.load(f"workspace/{file_root}_color_latents.npy"))
            # else:
            color_latents = drop_selection
        ...
        if render_track is None:
            for t, t_bpm, t_dur, t_start, t_stop in tracks:
                ...
                latents[transin:transout] = (1 - linsp[:, None]) * latents[transin:transout] + linsp[
                    :, None
                ] * transition
        ...
        latents = th.from_numpy(gaussian_filter(latents, [3 * smf, 0, 0]))
        ...
        noise_vec = np.random.normal(size=(int(len(latents) // num_loops), 1, min_h * 32, min_w * 32))
        loop_len = len(noise_vec)
        num_loops = int(len(latents) // loop_len)

        noise_noisy = th.from_numpy(gaussian_filter(noise_vec, [1 * smf, 0, 0, 0], mode="wrap")).float().cuda()
        ...
        for ns in range(0, 9):
            ...
            low_noise = F.interpolate(noise_vox, size=(2 ** ns * min_h, 2 ** ns * min_w)).cpu().numpy()
            base_noise = F.interpolate(noise_smooth, size=(2 ** ns * min_h, 2 ** ns * min_w)).cpu().numpy()

            noise_scale = np.zeros((len(latents), *base_noise.shape[1:]))
            for idx in range(0, len(noise_scale), loop_len):
                len_ns = len(noise_scale[idx : idx + loop_len])
                noise_scale[idx : idx + loop_len] = high_noise_mod[idx : idx + loop_len] * high_noise[:len_ns]
                noise_scale[idx : idx + loop_len] += (1 - high_noise_mod[idx : idx + loop_len]) * base_noise[:len_ns]
                noise_scale[idx : idx + loop_len] += low_noise_mod[idx : idx + loop_len] * low_noise[:len_ns]
